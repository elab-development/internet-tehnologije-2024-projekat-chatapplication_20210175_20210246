...
class ChatMessageController extends Controller
{
    ...
    public function destroy(DeleteMessageRequest $request): JsonResponse
    {
        ...
        if (!$message) {
            return $this->error("Poruka nije pronađena", 404);
        }

        #Proveri da li je korisnik vlasnik poruke
        if ($message->user_id !== auth('api')->user()->id) {
            return $this->error("Nemate pravo da obrišete ovu poruku", 403);
        }

        $message->delete();

        return $this->success([], "Poruka je uspešno obrisana");
    }
